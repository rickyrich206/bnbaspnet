        ...
        tableExport: function (options) {
            ...
            if (defaults.type == 'csv' || defaults.type == 'tsv' || defaults.type == 'txt') {
                    ...
                    $rows.each(function () {
                        ...
                        ForEachVisibleCell(this, rowselector, rowIndex, length + $rows.length,
                                function (cell, row, col) {
                                    if (($(cell).hasClass("export-visible")) || $(cell).data("tableexport-display") == 'always' ||
                        ($(cell).css('display') != 'none' &&
                         $(cell).css('visibility') != 'hidden' &&
                         $(cell).data("tableexport-display") != 'none')) {
                                        if (!$(cell).hasClass("export-hidden")) {
                ...
                $tables.each(function () {
                    ...
                    else if (MSDocType === "word") {
                        docData += '<table style="border-collapse: collapse;"><thead>';
                    }

                    // Header row
                    docData += defaults.header;
                        ...
                        ForEachVisibleCell(this, 'th,td', rowIndex, $hrows.length,
                          function (cell, row, col) {
                              if (cell !== null) {
                                  ...
                                  if (thstyle !== '') {
                                      trData += ' ' + thstyle + '"';
                                      //if ($(cell)[rowIndex].id === undefined && $(cell)[rowIndex].id != 'thReportEmptyHeader')
                                      trData += ' border : 1px solid black ';
                                  }
                                  else {
                                      //if ($(cell)[rowIndex].id === undefined && $(cell)[rowIndex].id != 'thReportEmptyHeader')
                                      trData += ' style="border : 1px solid black" ';
                                  }
                                  //   
                                  if ($(cell).is("[colspan]"))
                          ...
                          function (cell, row, col) {
                              if (cell !== null) {
                                  ...
                                  if ($(cell).is("[rowspan]"))
                                      trData += ' rowspan="' + $(cell).attr('rowspan') + '"';
                                  if ($(cell).is("[style]")) {
                                      var colstyle = 'border : 1px solid black';
                                      if (!$(cell).hasClass("viewReport")) {
                                          trData += ' style="' + $(cell).attr('style') + colstyle + '"';
                                      }
                                      else {
                                          trData += ' style="' + $(cell).attr('style') + '"';
                                      }
                                  }
                                  else {
                                      if (!$(cell).hasClass("viewReport")) {
                ...
                var nodeList = $(el)[0];
                $hrows = $(nodeList).find('thead').first().find('tr');
                $rows = $(nodeList).find('tbody').first().find('tr');

                $hrows.each(function (rowIndex, item) {
                    if ($(item).find('.export-hidden').length > 0) {
                        $(item).find('.export-hidden').remove();
                    }
                    if ($(item).find('.export-visible').length > 0) {
                        $(item).find('.export-visible').css('display', 'block');
                        $(item).removeClass('.export-visible');
                    }
                });

                $rows.each(function (inde, item) {
                    if ($(item).find('.export-hidden').length > 0) {
                        $(item).find('.export-hidden').remove();
                    }
                    if ($(item).find('.export-visible').length > 0) {
                ...
                //require(["pdfdebug"], function (pdfdebug1) {
                html2canvas(nodeList, window).then(
                      function (canvas) {
                ...
                if (defaults.pdfmake.enabled === true) {
                    ...
                    if (typeof teOptions.images != 'undefined') {
                        $(el).filter(function () {
                            return $(this).hasClass("export-visible") || $(this).data("tableexport-display") != 'none' &&
                               ($(this).is(':visible') ||
                                $(this).data("tableexport-display") == 'always' ||
            ...
            function ForEachVisibleCell(tableRow, selector, rowIndex, rowCount, cellcallback) {
                if ($.inArray(rowIndex, defaults.ignoreRow) == -1 &&
                    ...
                    $row.each(function (colIndex) {
                        if (($(this).hasClass("export-visible")) || $(this).data("tableexport-display") == 'always' ||
                            ($(this).css('display') != 'none' &&
                             $(this).css('visibility') != 'hidden' &&
                             $(this).data("tableexport-display") != 'none')) {
                            if (!$(this).hasClass("export-hidden")) {
